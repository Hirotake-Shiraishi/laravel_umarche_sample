...
class Product extends Model
{
    ...
    /**
     * 在庫あり商品のみに絞るスコープ（指摘#1 修正済み）
     * 【指摘】having('quantity', '>', 1) だと在庫1個の商品が一覧から消えてしまう。
     * t_stocks は入出庫の履歴テーブルで sum(quantity) が実在庫数。
     * 「1以上であれば表示」が正しいため、> 0 に修正。
     */
    public function scopeAvailableItems($query)
    {
        $stocks = DB::table('t_stocks')
            ...
            ->groupBy('product_id')
            ->having('quantity', '>', 0);  // 修正: 1 → 0（在庫1個でも表示する）
    ...
    public function scopeSortOrder($query, $sortOrder)
    {
        ...
        return $query;  // 修正: 想定外値でもチェーンが切れないように追加
    }
